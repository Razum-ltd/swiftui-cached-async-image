...
public struct CachedAsyncImage<Content>: View where Content: View {
    ...
    public init(url: URL?, urlCache: URLCache = .shared, sessionDelegate: (any URLSessionDelegate)? = nil, scale: CGFloat = 1) where Content == Image {
        let urlRequest = url == nil ? nil : URLRequest(url: url!)
      self.init(urlRequest: urlRequest, urlCache: urlCache, sessionDelegate: sessionDelegate, scale: scale)
    }
    ...
    ///     displays. For example, set a value of `2` for an image that you
    ///     would name with the `@2x` suffix if stored in a file on disk.
    public init(urlRequest: URLRequest?, urlCache: URLCache = .shared, sessionDelegate: (any URLSessionDelegate)? = nil, scale: CGFloat = 1) where Content == Image {
      self.init(urlRequest: urlRequest, urlCache: urlCache, sessionDelegate: sessionDelegate, scale: scale) { phase in
#if os(macOS)
    ...
    public init<I, P>(url: URL?, urlCache: URLCache = .shared, sessionDelegate: (any URLSessionDelegate)? = nil, scale: CGFloat = 1, @ViewBuilder content: @escaping (Image) -> I, @ViewBuilder placeholder: @escaping () -> P) where Content == _ConditionalContent<I, P>, I : View, P : View {
        let urlRequest = url == nil ? nil : URLRequest(url: url!)
      self.init(urlRequest: urlRequest, urlCache: urlCache, sessionDelegate: sessionDelegate, scale: scale, content: content, placeholder: placeholder)
    }
    ...
    ///   - placeholder: A closure that returns the view to show until the
    ///     load operation completes successfully.
    public init<I, P>(urlRequest: URLRequest?, urlCache: URLCache = .shared, sessionDelegate: (any URLSessionDelegate)? = nil, scale: CGFloat = 1, @ViewBuilder content: @escaping (Image) -> I, @ViewBuilder placeholder: @escaping () -> P) where Content == _ConditionalContent<I, P>, I : View, P : View {
      self.init(urlRequest: urlRequest, urlCache: urlCache, sessionDelegate: sessionDelegate, scale: scale) { phase in
            if let image = phase.image {
                content(image)
            } else {
    ...
    public init(url: URL?, urlCache: URLCache = .shared, sessionDelegate: (any URLSessionDelegate)? = nil, scale: CGFloat = 1, transaction: Transaction = Transaction(), @ViewBuilder content: @escaping (AsyncImagePhase) -> Content) {
        let urlRequest = url == nil ? nil : URLRequest(url: url!)
      self.init(urlRequest: urlRequest, urlCache: urlCache, sessionDelegate: sessionDelegate, scale: scale, transaction: transaction, content: content)
    }
    ...
    ///   - content: A closure that takes the load phase as an input, and
    ///     returns the view to display for the specified phase.
  public init(urlRequest: URLRequest?, urlCache: URLCache = .shared, sessionDelegate: (any URLSessionDelegate)? = nil, scale: CGFloat = 1, transaction: Transaction = Transaction(), @ViewBuilder content: @escaping (AsyncImagePhase) -> Content) {
